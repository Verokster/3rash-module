...
namespace Resolution
{
	...
	{
		memset(resolutionsList, NULL, sizeof(ThrashResolution) * resolutionsListCount);
		...
		desktopMode.colorDepth = devMode.dmBitsPerPel;

		BOOL is16bitModes = TRUE;
		BOOL is32bitModes = TRUE;
		if (about.flags & About_Windowed)
		{
			if (desktopMode.colorDepth == 16)
				is32bitModes = FALSE;
			else
				is16bitModes = FALSE;
		}
		...
		{
			if (devMode.dmPelsWidth >= 640 && devMode.dmPelsHeight >= 480 && (is16bitModes && devMode.dmBitsPerPel == 16 || is32bitModes && devMode.dmBitsPerPel == 32))
			{
		...
		if (!appWindowed)
		{
			...
			else
			{
				...
				if (res != DISP_CHANGE_SUCCESSFUL)
					Main::ShowError("Bad display mode", __FILE__, __FUNCTION__, __LINE__);
